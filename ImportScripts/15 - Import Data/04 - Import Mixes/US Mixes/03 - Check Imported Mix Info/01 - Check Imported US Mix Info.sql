...
Select	Plants.Name,
		...
		Case
			...
			Then -1.0
			When MixRecipe.QuantityUnitName In ('ML/CKg CM', 'ML/CKg', 'ml/100kg')
			Then Round(MaterialRecipe.QUANTITY * 100.0 * 1000.0 / (Material.SPECGR * 1.0 * CementitiousInfo.TotalQuantity), 6)
			...
			When	MixRecipe.QuantityUnitName In ('LB', 'Pounds', 'EA', 'Each') Or
					MixRecipe.QuantityUnitName In ('Per Cubic Yard') And (Material.FamilyMaterialTypeID <> 3 Or IsNull(Material.MOISTURE, 0.0) = 0.0)
			Then Round(MaterialRecipe.QUANTITY * 2.204622476, 6)
			When	MixRecipe.QuantityUnitName In ('OZ', 'LQ OZ', 'Ounces', 'fl oz', 'Fluid Oz', 'BG', 'Bag', 'Bags', 'DS', 'FO') Or
					MixRecipe.QuantityUnitName In ('Per Cubic Yard') And Material.FamilyMaterialTypeID = 3 And IsNull(Material.MOISTURE, 0.0) > 0.0
			Then Round(MaterialRecipe.QUANTITY / (Material.SPECGR * 0.0651984837440621 * 0.45359240000781), 6)
			When MixRecipe.QuantityUnitName In ('KG', 'Kilograms')
			...
			When MixRecipe.QuantityUnitName In ('ML', 'CC', 'Ml-liter', 'ml/m3', 'Milliliters')
...
Where	Isnull(@CheckForMatchingRecipeQuantities, 0) = 0 And
		...
		Case
			...
			When	MixRecipe.QuantityUnitName In ('LB', 'Pounds', 'EA', 'Each') Or
					MixRecipe.QuantityUnitName In ('Per Cubic Yard') And (Material.FamilyMaterialTypeID <> 3 Or IsNull(Material.MOISTURE, 0.0) = 0.0)
			Then Round(MaterialRecipe.QUANTITY * 2.204622476, 6)
			When	MixRecipe.QuantityUnitName In ('OZ', 'LQ OZ', 'Ounces', 'fl oz', 'Fluid Oz', 'Bags', 'Bag', 'BG', 'DS', 'FO') Or
					MixRecipe.QuantityUnitName In ('Per Cubic Yard') And Material.FamilyMaterialTypeID = 3 And IsNull(Material.MOISTURE, 0.0) > 0.0
			Then Round(MaterialRecipe.QUANTITY / (Material.SPECGR * 0.0651984837440621 * 0.45359240000781), 6)
			When MixRecipe.QuantityUnitName In ('KG', 'Kilograms')
			Then Round(MaterialRecipe.QUANTITY, 6)
