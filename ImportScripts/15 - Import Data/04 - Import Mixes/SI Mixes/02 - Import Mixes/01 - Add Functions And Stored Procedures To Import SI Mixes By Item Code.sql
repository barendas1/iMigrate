    ...
    Begin try
		...
		Begin
		    ...
		    Begin
		        ...
		        From #MixRecipeInfo As MixRecipeInfo
		        Where MixRecipeInfo.QuantityUnit In ('LQ OZ', 'OZ', 'Ounces', 'fl oz', 'Fluid Oz')
		        ...
		        From #MixRecipeInfo As MixRecipeInfo
		        Where MixRecipeInfo.QuantityUnit In ('ML', 'Ml-liter')

				Exec dbo.Utility_DisplayConsoleMessage 'Convert Recipe Quantities From mL/m3 to Kgs'

				Update MixRecipeInfo 
					Set MixRecipeInfo.ConvertedQuantity = MixRecipeInfo.Quantity * MixRecipeInfo.SpecificGravity * 1.0 / 1000.0
				From #MixRecipeInfo As MixRecipeInfo
				Where MixRecipeInfo.QuantityUnit In ('mL/m3', 'ml/m3', 'mL/m^3', 'ml/m^3')
		        ...
		        Exec dbo.Utility_DisplayConsoleMessage 'Treat Units Of Bags And Dosages As Mls And Convert Recipe Quantities From Mls to Kgs'
				...
				Exec dbo.Utility_DisplayConsoleMessage 'Treat Units Of Each As Kgs'
				...
				Update MixRecipeInfo 
					Set MixRecipeInfo.ConvertedQuantity = MixRecipeInfo.Quantity * MixRecipeInfo.SpecificGravity * 0.00001 * CementitiousInfo.TotalQuantity
				From #MixRecipeInfo As MixRecipeInfo
				Inner Join
				(
					Select MixRecipeInfo.MixID, Sum(Isnull(MixRecipeInfo.ConvertedQuantity, 0.0)) As TotalQuantity
				...
				On MixRecipeInfo.MixID = CementitiousInfo.MixID
				Where MixRecipeInfo.QuantityUnit In ('mL/100kg CM')


				Update MixRecipeInfo 
					Set MixRecipeInfo.ConvertedQuantity = MixRecipeInfo.Quantity * MixRecipeInfo.SpecificGravity * 0.00001 * CementInfo.TotalQuantity
					From #MixRecipeInfo As MixRecipeInfo
					Inner Join
					(
						Select MixRecipeInfo.MixID, Sum(Isnull(MixRecipeInfo.ConvertedQuantity, 0.0)) As TotalQuantity
						From #MixRecipeInfo As MixRecipeInfo
						Where MixRecipeInfo.FamilyMaterialTypeID = 2  -- Assuming 2 represents cement
						Group By MixRecipeInfo.MixID
						Having Round(Sum(Isnull(MixRecipeInfo.ConvertedQuantity, 0.0)), 2) > 0.0
					) As CementInfo
					On MixRecipeInfo.MixID = CementInfo.MixID
					Where MixRecipeInfo.QuantityUnit In ('mL/100kg C')
